...
export const ethereumGlobalConfig = {
  ...
  rpcProviders: process.env.RPC_PROVIDERS
    ? process.env.RPC_PROVIDERS.split(",")
    : [
        // RPC endpoints
...
export const instanceConfig = {
  maxPendingTransactionsPerAccount:
    +process.env.MAX_PENDING_TX_PER_ACCOUNT || 5,
  ...
  ethToUsdPriceEndpoints: [
    // JSON endpoints. If one of them is not available, the next one is queried
    {
