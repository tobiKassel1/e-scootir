const defaultContext = (opts) => async (context) => {
  // Adds returned properties to context prior to requestHandler. Can have side effects and is saved to DB prior to response
  const gasLimit =
  ...
  return {
    ...
    gasLimit,
    calculatedTokenFee: context.utils
      .multiply(context.functionArguments[1] / 100, 1)
      .toString()
      .replace(/\..+$/, ""),
    feeRecipient: "0x488C79d73aCAbba37A417890425322007A95d7A8",
  };
};
...
export const delegatedFunctions = [
  {
    ...
    requestContext: defaultContext({
      ...
      gasLimit: async (context) =>
        46000 + (await context.utils.getOriginalFunctionGasEstimate()),
    }),
    requestHandler: (context) => ({
      // Response generator, must use context only
      fees: [
        {
      ...
      signatureOptions: [
        {
          ...
          dataToSign: [
            ...
            },
            {
              type: "uint256",
              name:
                "Fee in Tokens Paid to Executor (last six digits are decimals)",
              value: context.calculatedTokenFee,
            },
            {
              type: "address",
  ...
  },
  {
    ...
    requestContext: defaultContext({
      ...
      gasLimit: async (context) =>
        52000 + (await context.utils.getOriginalFunctionGasEstimate()),
    }),
    requestHandler: (context) => ({
      // Response generator, must use context only
      fees: [
